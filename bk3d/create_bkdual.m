...
for vol = 1:size(bkgrid.elem,1)
    ...
    for edge = list_edges 
        if edge ~= 0 
            ...
            [nref] = is_split_by_plane(local_coords(ref,:), plane_normal , ...
                plane_center);
...
for vol = boundary_element
    ...
    % as arestas que devem ser incluidas compartilham um no com alguma das
    % arestas do volume de controle
    %dual_vol_face_index
%     ref1 = dual_vol_face_index(:,2) == 0;
%     ref2 = dual_vol_face_index(:,3) == 0;
%     ref = dual_vol_face_index(:,1) ~= vol;
%     ref = ref &  ref_edges_include;


%     
%     ref = any(dual_vol_face_index(:,1:2) == 0,2);
%     boundary_vol_neighbours = intersect(boundary_element, vol_neighbours);
%     auxmat = ismember(dual_vol_face_index(:,1:3),boundary_vol_neighbours);
%     auxmat = auxmat(:,1) & (auxmat(:,2) | auxmat(:,3));
%     auxmat = auxmat & ref & ref_edges_include;
