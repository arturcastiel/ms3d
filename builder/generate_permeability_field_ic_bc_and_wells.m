function wells = ...
    generate_permeability_field_ic_bc_and_wells(mesh,preconfig)
% generating permeability field

%% generate node pressure
[p_node_dirichlet] = map_vec(mesh.vertex.flag,flagstruct.flag_value_dirichlet);
[p_node_neumman] = map_vec(mesh.vertex.flag,flagstruct.flag_value_neuman);

source_bface = map_vec(mesh.face.bound.flag, flagstruct.flag_value_neuman);
num_inner = size(mesh.face.inner.centroid,1);
num_bound = size(mesh.face.bound.centroid,1);
mesh.vertex.flag

% transforming generating boundary flags conditions
